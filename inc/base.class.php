...
class ZO_Base
{
    ...
    public static function getPageTitle(){
        ...
        if (!is_archive()){
            ...
            if(is_page()) :
                /* custom title. */
                if(isset($zo_meta->_zo_page_title_text) && !empty($zo_meta->_zo_page_title_text)):
            ...
            elseif (is_front_page() || is_home()):
                esc_html_e( 'Blog', 'lucian' );
            /* search */
            elseif (is_search()):
                printf( esc_html__( 'Search Results for: %s', 'lucian' ), '<span>' . get_search_query() . '</span>' );
            /* 404 */ 
            elseif (is_404()):
                esc_html_e( '404', 'lucian');
            /* other */
            else :
                the_title();
            endif;
            ...
            /* author. */
            elseif ( is_author() ) :
                printf( esc_html__( 'Author: %s', 'lucian' ), '<span class="vcard">' . get_the_author() . '</span>' );
            /* date */
            elseif ( is_day() ) :
                printf( esc_html__( 'Day: %s', 'lucian' ), '<span>' . get_the_date() . '</span>' );
            elseif ( is_month() ) :
                printf( esc_html__( 'Month: %s', 'lucian' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'lucian' ) ) . '</span>' );
            elseif ( is_year() ) :
                printf( esc_html__( 'Year: %s', 'lucian' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'lucian' ) ) . '</span>' );
            /* post format */
            elseif ( is_tax( 'post_format', 'post-format-aside' ) ) :
                esc_html_e( 'Asides', 'lucian' );
            elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) :
                esc_html_e( 'Galleries', 'lucian');
            elseif ( is_tax( 'post_format', 'post-format-image' ) ) :
                esc_html_e( 'Images', 'lucian');
            elseif ( is_tax( 'post_format', 'post-format-video' ) ) :
                esc_html_e( 'Videos', 'lucian' );
            elseif ( is_tax( 'post_format', 'post-format-quote' ) ) :
                esc_html_e( 'Quotes', 'lucian' );
            elseif ( is_tax( 'post_format', 'post-format-link' ) ) :
                esc_html_e( 'Links', 'lucian' );
            elseif ( is_tax( 'post_format', 'post-format-status' ) ) :
                esc_html_e( 'Statuses', 'lucian' );
            elseif ( is_tax( 'post_format', 'post-format-audio' ) ) :
                esc_html_e( 'Audios', 'lucian' );
            elseif ( is_tax( 'post_format', 'post-format-chat' ) ) :
                esc_html_e( 'Chats', 'lucian' );
            /* woocommerce */
    ...
    public static function getBreadCrumb($separator = '') {
        ...
        /* search */
        if( is_search() ){ echo '<li>'.esc_html__("Search", 'lucian').'</li>'; }
        /* date */
        if( is_year() ){ echo '<li>'.get_the_time('Y').'</li>'; }
        /* 404 */
        if( is_404() ) {
            echo '<li>'.esc_html__("404 - Page not Found", 'lucian').'</li>';
        }
        /* archive */
